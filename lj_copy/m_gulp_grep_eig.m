function eigvec = m_gulp_grep_eig(kpt,NUM_ATOMS_UCELL)
%--------------------------------------------------------------------------
%eigvec = gulp_lj_eig(kpt,...
%        NUM_MODES,NUM_ATOMS_UCELL,...
...
%        'commentStyle', '--'); 
%    dummy = textscan(fid,'%f%f%f%f%f%f%f'); 
%    fclose(fid);
%    system('rm eigvec4.dat'); 

dummy = load('eigvec4.dat');
...
eigvec = zeros(3*NUM_ATOMS_UCELL,3*NUM_ATOMS_UCELL); 
    for imode = 1:(3*NUM_ATOMS_UCELL/3)
        eigvec(:,(imode-1)*3+1) =...
            dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,2)...
            + i*dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,3);
        eigvec(:,(imode-1)*3+2) =...
            dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,4)...
            + i*dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,5);
        eigvec(:,(imode-1)*3+3) =...
            dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,6)...
            + i*dummy((imode-1)*3*NUM_ATOMS_UCELL+1:(imode)*3*NUM_ATOMS_UCELL,7);
    end
end
